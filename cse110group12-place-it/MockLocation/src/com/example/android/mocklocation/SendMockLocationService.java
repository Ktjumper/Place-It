 ...
 * back-end. All other apps that are connected to Location Services will see the
 * test location values instead of real values, until the test is over.
 * 
 * To use this service, define the mock location values you want to use in the
...
public class SendMockLocationService extends Service implements
	 ...
	 * onStartCommand() via a Message to the Handler. The object makes it
	 * possible to pass the parameters through the predefined Message field
	 * Message.obj.
	 */
	...
	// Stores an instance of the object that dispatches work requests to the
	// worker thread
	...
	// The type of test requested, either ACTION_START_ONCE or
	// ACTION_START_CONTINUOUS
	private String mTestRequest;

	/**
	 * Define a class that manages the work of injecting test locations, using
	 * the Android Handler API. A Handler facilitates running the work on a
	 * separate thread, so that the test loop doesn't block the UI thread.
	 * 
	 * A Handler is an object that can run code on a thread. Handler methods
	 * allow you to associate the object with a Looper, which dispatches Message
	 * objects to the Handler code. In turn, Message objects contain data and
	 * instructions for the Handler's code. A Handler is created with a default
	 * thread and default Looper, but you can inject the Looper from another
	 * thread if you want. This is often done to associate a Handler with a
	 * HandlerThread thread that runs in the background.
	 */

	public class UpdateHandler extends Handler {

		/**
		 * Create a new Handler that uses the thread of the HandlerThread that
		 * contains the provided Looper object.
		 * 
		 * @param inputLooper
		 *            The Looper object of a HandlerThread.
		 */
		public UpdateHandler(Looper inputLooper) {
			// Instantiate the Handler with a Looper connected to a background
			// thread
			super(inputLooper);
		...
		/*
		 * Do the work. The Handler's Looper dispatches a Message to
		 * handleMessage(), which then runs the code it contains on the thread
		 * associated with the Looper. The Message object allows external
		 * callers to pass data to handleMessage().
		 * 
		 * handleMessage() assumes that the location client already has a
		...
		public void handleMessage(Message msg) {
			...
			Location mockLocation = new Location(
					LocationUtils.LOCATION_PROVIDER);
			...
			if (!mTestStarted) {
				...
				/*
				 * Wait to allow the test to switch to the app under test, by
				...
				/*
				 * Run the test loop, iterating through the array of test
				 * locations. Each test location is injected into Location
				 * Services, after which the thread is put to sleep for the
				 * requested interval.
				 * 
				 * Uses a "do" loop so that one-time test and continuous test
				 * can share code.
				 */
				do {
					for (int index = 0; index < mLocationArray.length; index++) {
						 ...
						 * UTC timezone must be specified.
						 */
						...
						mockLocation
								.setAccuracy(mLocationArray[index].Accuracy);
						mockLocation
								.setLatitude(mLocationArray[index].Latitude);
						mockLocation
								.setLongitude(mLocationArray[index].Longitude);
						...
						/*
						 * Change the elapsed uptime and clock time by the
					...
					/*
					 * Run the "do" while "testOnce" is false. For a one-time
					 * test, testOnce is true, so the "do" loop runs only once.
				...
				/*
				 * The Service received a request to start testing, but a test
				 * was already in progress. Send a message back to the main
				 * Activity, and ignore the request.
				 */
	...
	/*
	 * At startup, load the static mock location data from
	 * MockLocationConstants.java, then create a HandlerThread to inject the
	 * locations and start it.
	 */
	@Override
	public void onCreate() {
		List<Location> gpxList = LocationUtils.decodeGPX(getResources()
				.openRawResource(R.raw.locations));

		double[] latitudes = new double[gpxList.size()];
		double[] longitudes = new double[gpxList.size()];
		float[] accuracies = new float[gpxList.size()];
		for (int i = 0; i < gpxList.size(); i++) {
			latitudes[i] = gpxList.get(i).getLatitude();
			longitudes[i] = gpxList.get(i).getLongitude();
			accuracies[i] = gpxList.get(i).getAccuracy();
		}

		/*
		 * Load the mock location data from MockLocationConstants.java
		 */
		mLocationArray = buildTestLocationArray(latitudes, longitudes,
				accuracies);
		 ...
		 * are only available within the this app.
		 */
		mLocalBroadcastManager = LocalBroadcastManager.getInstance(this);

		/*
		 * Create a new background thread with an associated Looper that
		 * processes Message objects from a MessageQueue. The Looper allows test
		 * Activities to send repeated requests to inject mock locations from
		 * this Service.
		 */
		mWorkThread = new HandlerThread("UpdateThread",
				Process.THREAD_PRIORITY_BACKGROUND);

		/*
		 * Start the thread. Nothing actually runs until the Looper for this
		 * thread dispatches a Message to the Handler.
		 */
		mWorkThread.start();
		...
		/*
		 * Create a Handler object and pass in the Looper for the thread. The
		 * Looper can now dispatch Message objects to the Handler's
	...
	/**
	 * Post a notification to the notification bar. The title of the
	 * notification is fixed, but the content comes from the input argument
	 * contentText. The notification does not contain a content Intent, because
	 * the only destination it could have would be the main activity. It's
	 * better to use the Recents button to go to the existing main activity.
	 * 
	 * @param contentText
	 *            Text to use for the notification content (main line of
	...
	private void postNotification(String contentText) {
		...
		/*
		 * Post the notification. All notifications from
		 * InjectMockLocationService have the same ID, so posting a new
		 * notification overwrites the old one.
		 */
	...
	/*
	 * Since onBind is a static method, any subclass of Service must override
	 * it. However, since this Service is not designed to be a bound Service, it
	 * returns null.
	 */
	@Override
	...
	/*
	 * Respond to an Intent sent by startService. onCreate() is called before
	 * this method, to take care of initialization.
	 * 
	...
	public int onStartCommand(Intent startIntent, int flags, int startId) {
		...
		if ((TextUtils.equals(mTestRequest, LocationUtils.ACTION_START_ONCE))
			...
			mPauseInterval = startIntent.getIntExtra(
					LocationUtils.EXTRA_PAUSE_VALUE, 2);
			mInjectionInterval = startIntent.getIntExtra(
					LocationUtils.EXTRA_SEND_INTERVAL, 1);
		...
		} else if (TextUtils.equals(mTestRequest,
				LocationUtils.ACTION_STOP_TEST)) {

			// Remove any existing notifications
			removeNotification();

			// Send a message back to the main activity that the test is
			// stopping
	 ...
	 * @param accuracy_array
	 *            An array of accuracy values
	 * 
	 * @return An array of test location data
	 */
	private TestLocation[] buildTestLocationArray(double[] lat_array,
			double[] lng_array, float[] accuracy_array) {

		// Temporary array of location data
		TestLocation[] location_array = new TestLocation[lat_array.length];

		/*
		 * Iterate through all the arrays of data. This loop assumes that the
		 * arrays all have the same length.
		 */
		for (int index = 0; index < lat_array.length; index++) {

			/*
			 * For each location, create a new location storage object. Set the
			 * "provider" value to a number that identifies the location. This
			 * allows the tester or the app under test to identify a particular
			 * location
			 */
			location_array[index] = new TestLocation(Integer.toString(index),
					lat_array[index], lng_array[index], accuracy_array[index]);
		}
	...
	public void onConnectionFailed(ConnectionResult result) {
		...
		sendBroadcastMessage(LocationUtils.CODE_CONNECTION_FAILED,
				result.getErrorCode());
	...
	/**
	 * Send a broadcast message back to the main Activity, indicating a change
	 * in status.
	 * 
	 * @param code1
	 *            The main status code to return
	 * @param code2
	 *            A subcode for the status code, or 0.
	 */
	...
	/*
	 * When the client is connected, Location Services calls this method, which
	 * in turn starts the testing cycle by sending a message to the Handler that
	 * injects the test locations.
	 */
	@Override
	public void onConnected(Bundle arg0) {
		...
		TestParam testParams = new TestParam(mTestRequest, mPauseInterval,
				mInjectionInterval);
